...
export class Cliente_AuthService {

  $user = new BehaviorSubject<UserModel | undefined>(undefined);

  constructor(private http:HttpClient) { }

  registro(request:any):Observable<LoginResponse>{
    return this.http.post<LoginResponse>(`${environment.apiUrl}/cliente/auth/registro`,request);
  }

  Login(request:any):Observable<LoginResponse>{
    return this.http.post<LoginResponse>(`${environment.apiUrl}/cliente/auth/login`,{
      Email: request.email,
